...
public class Shoot_FirstPerson : MonoBehaviour
{
    ...
    private IEnumerator waitForShoot(float time)
    {
        ...
        if (rigidbody)
        {
            ...
            Vector3 centerPositionOnNearPlane = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width * 0.5f, Screen.height * 0.5f, Camera.main.nearClipPlane));//畫面中央
            go.transform.position = transform.position;//從start的位置射出
            go.transform.LookAt(centerPositionOnNearPlane);//箭的方向
            arrow.GetComponent<ArrowRotation>().target = go;
            go.GetComponent<Rigidbody>().AddForce(Vector3.Normalize(centerPositionOnNearPlane - Camera.main.transform.position) * m_shootingForce * power);//終點方向*力量
        }
        else
